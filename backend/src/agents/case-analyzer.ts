 ...
 */
export async function runAgent3(query: UserQuery): Promise<CaseAnalysis> {
  console.log(`[Agent 3] Case Analyzer - Analyzing user query for ${query.brand}`);

  // Search for best matching case pattern
  const matchedPattern = await searchCasePattern(query);
  
  if (matchedPattern) {
    console.log(`[Agent 3] Found pattern: ${matchedPattern.issue_type} (${matchedPattern.success_rate} success rate)`);
  }

  // Search for similar signals (optional context)
  const similarSignals = await searchSimilarSignals(query);
  console.log(`[Agent 3] Found ${similarSignals.length} similar signals`);

  // Analyze with AI
  const analysis = await analyzeCase(query, matchedPattern, similarSignals);

  console.log(`[Agent 3] Analysis complete - Category: ${analysis.issueCategory}, Severity: ${analysis.severity}`);
