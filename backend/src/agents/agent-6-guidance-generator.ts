 ...
 */
export async function runAgent6(query: UserQuery, analysis: CaseAnalysis): Promise<Guidance> {
  console.log("[Agent 6] Guidance Generator - Generating guidance");
  ...
  console.log(`[Agent 6] Found ${contacts.length} contacts for ${query.brand} (purpose: ${purpose})`);

  // Generate guidance
  const guidance = await generateGuidance(query, analysis, contacts);

  console.log("[Agent 6] Guidance generation complete");
